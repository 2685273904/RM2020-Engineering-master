...
#define JUDGESYS_DATAID_DRAWCLIENTGRAPHIC  0x0100u     /* 上传客户端数据显示的内容ID */
...
#define JUDGESYS_DATALENGTH_CLIENTUPLOADINFO	 19//数据内容ID2+发送者ID2+客户端ID2+自定义浮点数据3*4+自定义八位数据1=19
#define JUDGESYS_DATALENGTH_DRAWCLIENTGRAPHIC	 61//55+6
...
#define JUDGESYS_FULLDATALENGTH_DRAWCLIENTGRAPHIC (JUDGESYS_DATALENGTH_DRAWCLIENTGRAPHIC + JUDGESYS_DATALENGTH_BASE)
//#define JUDGESYS_FULLDATALENGTH_ROBOTCOMINFO  	 (JUDGESYS_DATALENGTH_ROBOTCOMINFO + JUDGESYS_DATALENGTH_BASE)

#define JUDGESYS_USERINFO_TYPE_CLIENT     0x01
#define JUDGESYS_USERINFO_TYPE_ROBOT      0x02
#define JUDGESYS_USERINFO_TYPE_GRAPHIC    0x03
...
typedef enum{
  ROBOT_ID_HERO = 1,
	ROBOT_ID_ENGINEER = 2,
  ROBOT_ID_INFANTRY_3 = 3,
  ROBOT_ID_INFANTRY_4 = 4,
  ROBOT_ID_INFANTRY_5 = 5,
  ROBOT_ID_AERIAL = 6,
  ROBOT_ID_SENTRY = 7,
}JudgeSYS_Robot_ID_e;
...
/* 客户端自定义图形，用于显示在操作界面，限频 10Hz CmdID 0x0301 */
typedef struct{
	union {
		uint8_t dataBuff[JUDGESYS_DATALENGTH_DRAWCLIENTGRAPHIC];
		__packed struct{
			uint16_t data_cmd_id; /* 数据段的内容 ID  */
			uint16_t send_ID;			/* 发送者的 ID  */
			uint16_t receiver_ID; /* 接收者的 ID  */
...
typedef struct{
  ...
  JudgeSYS_ClientGraphicDraw_t ClientGraphicDraw;
  uint8_t lastSendType;         /* 由于自定义数据共用带宽，用于记录上次发送的数据类型 */
...
extern uint8_t JudgeSystem_rxBuff[JUDGESYSTEM_PACKSIZE]; 	//接收buff
...
void Judge_sendDataToClient(void);
